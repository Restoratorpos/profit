 ...
 *
 * A **port of `packages/auth/lib/countries.ts`**, not an import — the same
 * relationship `src/theme.ts` has with the design system's CSS. That package is
 * pulled in through `@repo/*` path aliases that Metro does not share, and it
 * hangs off React DOM components. The *table* is what matters, and it must stay
 * identical on both sides: a number assembled here is compared byte-for-byte
 * against one assembled at the desk.
 *
 * Keyed by ISO code, **not** dial code — Kazakhstan and Russia both dial +7, so
 * a dial-code key would collide and make the two indistinguishable in a picker.
 */

/** Bare digits, exactly as MySQL stores them. Identical to `normalizePhone` in
 * `packages/auth/lib/phone.ts` and `apps/backend/src/lib/phone.ts`. */
export const normalizePhone = (phone: string): string =>
  phone.replace(/\D/g, "");

export interface Country {
  /** ISO 3166-1 alpha-2. Unique, so it is what the picker stores. */
  code: string;
  /** Digits only, no "+". */
  dialCode: string;
...
/**
 * The flag, as an emoji built from the ISO code's regional-indicator pair.
 *
 * The desk app draws inline SVG instead, because Windows ships no country-flag
 * glyphs and `🇺🇿` renders there as the letters "UZ". Both phone platforms do
 * have them (iOS always, Android since Noto Color Emoji), so the cheap version
 * is the right one here — and the name and dial code sit beside it either way.
 */
 ...
 *
 * Display only. A number whose length matches no country we serve is returned
 * with just a `+`: showing the raw digits is honest, whereas grouping them
 * under a guessed country would invent structure that is not there.
 *
 * Cached because lists call it once per row and the same numbers recur across
 * re-renders; the set of them is bounded by the gym's roster.
 */
