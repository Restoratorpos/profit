...
export const CheckInDialog = ({
  ...
  const submit = () => {
    ...
    const handlers = {
      // No router.refresh(): the mutation invalidates the worker queries, so
      // the row's shift state and hours catch up on their own.
      onSuccess: () => onOpenChange(false),
      onError: (cause: Error) => setError(cause.message),
    };

    const variables = { at: at.toISOString(), workerId: worker.id };

    if (mode === "in") {
      checkIn.mutate(variables, handlers);

      return;
    }

    checkOut.mutate(variables, handlers);
  };
