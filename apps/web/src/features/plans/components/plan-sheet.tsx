...
const ResourceFields = ({
  ...
  halls: readonly NamedOption[];
  messages: Messages;
  /** Resolves to the created hall so the picker can select it immediately. */
  onCreateHall: (name: string) => Promise<NamedOption>;
    ...
    <Field>
      ...
      <CreatableCombobox
        ...
        onCreate={async (label) => {
          try {
            const hall = await onCreateHall(label);

            return { value: hall.id };
          } catch (cause) {
...
export const PlanSheet = ({
  ...
  /*
   * Pending state comes from the mutation rather than a parallel useState that
   * has to be flipped on every exit path — the old version had to remember to
   * clear it before both the success return and the error return.
   */
  const savePlan = useSavePlan();
  const createHall = useCreateHall();
  const isPending = savePlan.isPending;
  ...
  // Not async any more: `mutate` reports through its callbacks rather than a
  // promise, so there is nothing left here to await.
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    savePlan.mutate(
      { input: payload, planId: plan?.id },
      {
        // Closing is the success signal; the table behind refreshes itself
        // because the mutation invalidates the plans query.
        ...
        >
          <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 pb-4">
            ...
            <ResourceFields
              ...
              halls={halls}
              messages={messages}
              onCreateHall={createHall.mutateAsync}
              onHall={setHallId}
