...
import { loadProductMovementsAction } from "../actions";
...
export const ProductHistorySheet = ({
  ...
  useEffect(() => {
    ...
    // Guards against a slow response for a row the user already navigated away
    // from overwriting a faster one for the row they are looking at now.
    let isCurrent = true;

    setIsLoading(true);
    setMovements([]);

    loadProductMovementsAction(productId).then((result) => {
      if (!isCurrent) {
        return;
      }
