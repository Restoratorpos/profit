...
import { removeFaceAction, setFaceAction } from "@/lib/face-actions";
...
export const WorkerSheet = ({
  ...
  const savePerson = async (
    ...
    if (existingId) {
      const result = await updateWorkerAction(existingId, input);

      return { error: result.error, ok: result.ok, workerId: existingId };
    }

    const result = await createWorkerAction(input);
    const workerId = result.worker?.id ?? null;

    if (workerId) {
      setCreatedId(workerId);
    }

    return { error: result.error, ok: result.ok, workerId };
  };
  ...
  const saveFace = async (
    ...
    return await setFaceAction("worker", workerId, photo);
  };
  ...
  const handleRemoveFace = async () => {
    ...
    const result = await removeFaceAction("worker", workerId);
