...
import { loadWorkerPayrollAction, payWorkerAction } from "../actions";
...
interface PayWorkerDialogProperties {
  ...
  /** Called after a payment is recorded, so the list behind can catch up. */
  onPaid: () => void;
...
export const PayWorkerDialog = ({
  ...
  /*
   * The figures follow the month, so they are re-read whenever it changes
   * rather than derived from the row behind — that row was computed over the
   * list's range, which is not necessarily the month being settled.
   *
   * A slow reply for an old month must not overwrite a newer one.
   */
  useEffect(() => {
    if (!workerId) {
      return;
    ...
    loadWorkerPayrollAction(workerId, period).then((result) => {
      if (cancelled) {
        return;
      }
      ...
      if (result.ok && result.payroll) {
        setPayroll(result.payroll);
        return;
      }

      setPayroll(null);
      setError(result.error ?? messages["workers.payFailed"]);
    });

    return () => {
      cancelled = true;
    };
  }, [messages, period, workerId]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await payWorkerAction(workerId, {
      amount: String(value),
      method,
