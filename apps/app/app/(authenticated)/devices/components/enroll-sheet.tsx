...
import { enrollPersonAction, revokePersonAction } from "../actions";
...
interface EnrollSheetProperties {
  ...
  /** Re-reads the enrolled list after it changes. */
  onReload: () => void;
...
export const EnrollSheet = ({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await enrollPersonAction(device.id, {
      personId,
      personType,
      ...(photo ? { photo } : {}),
    });
  ...
  const handleRevoke = async (credentialId: string) => {
    ...
    const result = await revokePersonAction(device.id, credentialId);
