...
export const MemberSheet = ({
  ...
  const savePerson = async (
    ...
    if (existingId) {
      const result = await updateMemberAction(existingId, person);

      return { error: result.error, memberId: existingId, ok: result.ok };
    }
  ...
  const saveFace = async (
    ...
    return await setFaceAction("member", memberId, photo);
  };
  ...
  const handleRemoveFace = async () => {
    ...
    const result = await removeFaceAction("member", memberId);
