...
export const MembersView = ({
  ...
  const [page, setPage] = useState<MemberPage>(initial);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const next = await loadMembersAction(request);
  ...
  const handleDelete = async (member: MemberListItem) => {
    ...
    // Re-read rather than splice the row out: the counts in the header and the
    // page itself both move, and one of them being stale is how a screen starts
    // disagreeing with itself.
    setPage(await loadMembersAction(request));
  };
